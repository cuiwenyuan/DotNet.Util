# Wangcaisoft.DotNet.Util.Db.OleDb

> Access / OleDb 数据访问 / Access / OleDb data access

## 简介 / Introduction

`DotNet.Util.Db.OleDb` 在 `DotNet.Util.Db` 之上提供 Access / OleDb 的 `IDbHelper` 实现（`OleDbHelper`），可与 `DbUtil` 静态方法配合使用。

`DotNet.Util.Db.OleDb` provides the Access / OleDb `IDbHelper` implementation (`OleDbHelper`) on top of `DotNet.Util.Db`, usable with `DbUtil` static helpers.

## 包含内容 / What's Inside

- `OleDbHelper`：`DbHelper` / `IDbHelper` 的 Access / OleDb 实现（基于 `System.Data.OleDb`）。
- 复用 `DotNet.Util.Db` 的 `DbUtil` 静态 CRUD 助手与连接/事务管理。

## 安装 / Installation

```bash
dotnet add package Wangcaisoft.DotNet.Util.Db.OleDb
```
