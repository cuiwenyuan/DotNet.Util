...
namespace DotNet.Util
{
    ...
    public class ObjectPool<T> : DisposeBase, IPool<T> where T : class
    {
        ...
        private void StartTimer()
        {
            ...
            //绑定Elapsed事件
            _timer.Elapsed += new ElapsedEventHandler(Work);
            //设置执行一次（false）还是多次执行(true)
            _timer.AutoReset = true;
            //指示Timer是否应引发Elapsed事件。
            _timer.Enabled = true;
        }
        ...
        private void Work(Object source, ElapsedEventArgs e)
        {
            if (Interlocked.Exchange(ref _inTimer, 1) == 0)
            {
                ...
                LogUtil.WriteLog("触发的事件发生在： {0}", DateTime.Now.ToString(BaseSystemInfo.DateTimeLongFormat));
                // 遍历并干掉过期项
                var count = 0;
                ...
                if (count > 0 || ncount > 0 || fcount > 0)
                {
                    ...
                    WriteLog(
                        "Release New={6:n0} Release={7:n0} Free={0} Busy={1} 清除过期资源 {2:n0} 项。总请求 {3:n0} 次，命中 {4:p2}，平均 {5:n2}us",
                        FreeCount, BusyCount, count, Total, p, Cost * 1000, ncount, fcount);
                }
            }
        }
        #endregion
        ...
        {
            LogUtil.WriteLog(Name + "." + format + args, "ObjectPool");
        }
        #endregion
    }
