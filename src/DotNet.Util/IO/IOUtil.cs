...
{
    /// <summary>IO工具类 - 来自NewLife</summary>
    /// <remarks>
    ...
    public static class IOUtil
    {
        ...
        public static Byte[] ReadArray(this Stream des)
        {
            ...
            if (des.CanSeek && len > des.Length - des.Position) throw new Exception("ReadArray错误，变长数组长度为" + len + "，但数据流可用数据只有" + (des.Length - des.Position));

            if (len > 1024 * 2) throw new Exception("安全需要，不允许读取超大变长数组 " + len + ">" + (1024 * 2) + "");
