...
namespace DotNet.Business
{
    ...
    {
        #region public virtual int SetCancelled(object id, bool hasIsAudited = true, bool recordUser = false) 取消标志
        ...
        public virtual int SetCancelled(object id, bool hasIsAudited = true, bool recordUser = true, int tableVersion = 4)
        {
            var parameters = new List<KeyValuePair<string, object>>();
            parameters.Add(new KeyValuePair<string, object>(BaseUtil.FieldIsCancelled, 1));
        ...
        #region public virtual int SetCancelled(object[] ids, bool hasIsAudited = true, bool recordUser = true) 批量取消标志
        ...
        /// <param name="tableVersion">版本默认4为老版本</param>
        /// <returns>影响行数</returns>
        public virtual int SetCancelled(object[] ids, bool hasIsAudited = true, bool recordUser = true, int tableVersion = 4)
        {
            // 循环执行
            var result = 0;
            if (ids == null)
            ...
            else
            {
                ...
                {
                    result += SetCancelled(t, hasIsAudited, recordUser, tableVersion);
                }
            }
            if (result > 0)
            {
