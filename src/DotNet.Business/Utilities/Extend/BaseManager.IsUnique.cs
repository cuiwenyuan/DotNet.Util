...
namespace DotNet.Business
{
    ...
    public partial class BaseManager : IBaseManager
    {
        ...
        /// <returns>是否</returns>
        public virtual bool IsUnique(string fieldValue, string excludeId, string fieldName, bool checkUserCompany = false, int tableVersion = 4)
        {
            var result = false;
            //安全过滤一下
            ...
            //未删除
            sb.Append(" AND " + (tableVersion == 4 ? BaseUtil.FieldDeletionStateCode : BaseUtil.FieldDeleted) + " = 0 AND " + BaseUtil.FieldEnabled + " = 1 ");
            //当前用户所在公司或者系统公用数据
            if (checkUserCompany)
            {
        ...
        /// <param name="tableVersion">版本默认4为老版本</param>
        /// <returns>是否</returns>
        public virtual bool IsUnique(string field1Value, string field2Value, string excludeId, string field1Name, string field2Name, bool checkUserCompany = false, int tableVersion = 4)
        {
            var result = false;
            //安全过滤一下
        ...
        /// <param name="tableVersion">版本默认4为老版本</param>
        /// <returns>是否</returns>
        public virtual bool IsUnique(string field1Value, string field2Value, string field3Value, string excludeId, string field1Name, string field2Name, string field3Name, bool checkUserCompany = false, int tableVersion = 4)
        {
            var result = false;
            //安全过滤一下
            field1Value = dbHelper.SqlSafe(field1Value);
