...
namespace DotNet.Business
{
    ...
    {
        #region public virtual int UndoSetAudited(object id, bool hasIsCancelled = true, bool recordUser = true) 撤销审核标志
        ...
        public virtual int UndoSetAudited(object id, bool hasIsCancelled = true, bool recordUser = true, int tableVersion = 4)
        {
            var parameters = new List<KeyValuePair<string, object>>();
            parameters.Add(new KeyValuePair<string, object>(BaseUtil.FieldIsAudited, 0));
        ...
        #region public virtual int UndoSetAudited(object[] ids, bool hasIsCancelled = true, bool recordUser = true) 批量撤销审核标志
        ...
        /// <param name="tableVersion">版本默认4为老版本</param>
        /// <returns>影响行数</returns>
        public virtual int UndoSetAudited(object[] ids, bool hasIsCancelled = true, bool recordUser = true, int tableVersion = 4)
        {
            // 循环执行
            var result = 0;
            if (ids == null)
            ...
            else
            {
                ...
                {
                    result += UndoSetAudited(t, hasIsCancelled, recordUser, tableVersion);
                }
            }
