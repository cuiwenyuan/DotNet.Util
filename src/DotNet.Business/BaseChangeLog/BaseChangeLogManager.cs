...
namespace DotNet.Business
{
    ...
    public partial class BaseChangeLogManager : BaseManager, IBaseManager
    {
        ...
        public DataTable GetDataTableByPage(string companyId, string departmentId, string userId, string startTime, string endTime, string searchKey, out int recordCount, int pageNo = 1, int pageSize = 20, string sortExpression = BaseChangeLogEntity.FieldCreateTime, string sortDirection = "DESC", bool showDisabled = true, bool showDeleted = true)
        {
            ...
            {
                sb.Append(" AND " + BaseChangeLogEntity.FieldCreateTime + " >= '" + startTime + "'");
            }
            if (ValidateUtil.IsDateTime(endTime))
            {
                sb.Append(" AND " + BaseChangeLogEntity.FieldCreateTime + " <= DATEADD(s,-1,DATEADD(d,1,'" + endTime + "'))");
            }
        ...
        public DataTable GetDataTable(bool myCompanyOnly = true)
        {
            ...
            //return GetDataTable(sb.Put(), null, new KeyValuePair<string, object>(BaseChangeLogEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseChangeLogEntity.FieldDeleted, 0));
            ...
            return CacheUtil.Cache<DataTable>(cacheKey, () => GetDataTable(sb.Put(), null, new KeyValuePair<string, object>(BaseChangeLogEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseChangeLogEntity.FieldDeleted, 0)), true, false, cacheTime);
        }
