...
namespace DotNet.Business
{
    ...
    public partial class BaseRoleOrganizationManager : BaseManager, IBaseManager
    {
        ...
        public DataTable GetDataTableByPage(string companyId, string departmentId, string userId, string startTime, string endTime, string searchKey, out int recordCount, int pageNo = 1, int pageSize = 20, string sortExpression = BaseRoleOrganizationEntity.FieldCreateTime, string sortDirection = "DESC", bool showDisabled = true, bool showDeleted = true)
        {
            ...
            {
                sb.Append(" AND " + BaseRoleOrganizationEntity.FieldCreateTime + " >= '" + startTime + "'");
            }
            if (ValidateUtil.IsDateTime(endTime))
            {
                sb.Append(" AND " + BaseRoleOrganizationEntity.FieldCreateTime + " <= DATEADD(s,-1,DATEADD(d,1,'" + endTime + "'))");
            }
        ...
        public DataTable GetDataTable(bool myCompanyOnly = true)
        {
            ...
            //return GetDataTable(sb.Put(), null, new KeyValuePair<string, object>(BaseRoleOrganizationEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseRoleOrganizationEntity.FieldDeleted, 0));
            ...
            return CacheUtil.Cache<DataTable>(cacheKey, () => GetDataTable(sb.Put(), null, new KeyValuePair<string, object>(BaseRoleOrganizationEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseRoleOrganizationEntity.FieldDeleted, 0)), true, false, cacheTime);
        }
