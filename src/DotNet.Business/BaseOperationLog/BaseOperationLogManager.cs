...
namespace DotNet.Business
{
    ...
    public partial class BaseOperationLogManager : BaseManager
    {
        ...
        /// <returns>数据表</returns>
        public override DataTable GetDataTableByPage(string companyId, string departmentId, string userId, string startTime, string endTime, string searchKey, out int recordCount, int pageNo = 1, int pageSize = 20, string sortExpression = BaseOperationLogEntity.FieldCreateTime, string sortDirection = "DESC", bool showDisabled = true, bool showDeleted = true)
        {
        ...
        public DataTable GetDataTable(bool myCompanyOnly = true)
        {
            ...
            if (myCompanyOnly)
            {
                //sb.Append("(" + BaseOperationLogEntity.FieldUserCompanyId + " = 0 OR " + BaseOperationLogEntity.FieldUserCompanyId + " = " + UserInfo.CompanyId + ")");
            }
            //return GetDataTable(sb.Return(), null, new KeyValuePair<string, object>(BaseOperationLogEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseOperationLogEntity.FieldDeleted, 0));
            ...
            return CacheUtil.Cache<DataTable>(cacheKey, () => GetDataTable(sb.Return(), null, new KeyValuePair<string, object>(BaseOperationLogEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseOperationLogEntity.FieldDeleted, 0)), true, false, cacheTime);
        }
