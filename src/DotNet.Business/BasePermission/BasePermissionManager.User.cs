    ...
    public partial class BasePermissionManager : BaseManager, IBaseManager
    {
        ...
        public string[] GetUserIdsByPermissionId(string systemCode, string permissionId)
        {
            ...
            if (!string.IsNullOrEmpty(permissionId))
            {
                ...
                // 3.组织机构有某个操作权限。。
                sql = "SELECT ResourceId FROM " + tableName + " WHERE (ResourceCategory = 'BaseOrganization') AND (PermissionId = " + permissionId + ") AND (" + BaseModuleEntity.FieldDeleted + " = 0) AND (Enabled = 1) ";
                dt = Fill(sql);
