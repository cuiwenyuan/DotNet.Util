    ...
    public partial class BaseSequenceManager : BaseManager, IBaseManager
    {
        ...
        protected BaseSequenceEntity GetSequenceByLock(string fullName, int defaultSequence)
        {
            ...
            if (sequenceEntity == null)
            {
                ...
                {
                    // 被锁定的记录数
                    var lockCount = DbUtil.LockNoWait(DbHelper, BaseSequenceEntity.TableName, new KeyValuePair<string, object>(BaseSequenceEntity.FieldFullName, BaseSequenceEntity.TableName));
                    if (lockCount > 0)
                    {
