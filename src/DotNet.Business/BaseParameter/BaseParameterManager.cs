...
namespace DotNet.Business
{
    ...
    public partial class BaseParameterManager : BaseManager, IBaseManager
    {
        ...
        /// <returns>数据表</returns>
        public DataTable GetDataTableByPage(string companyId, string departmentId, string userId, string startTime, string endTime, string searchKey, out int recordCount, int pageNo = 1, int pageSize = 20, string sortExpression = BaseParameterEntity.FieldCreateTime, string sortDirection = "DESC", bool showDisabled = true, bool showDeleted = true)
        {
            ...
            {
                sb.Append(" AND " + BaseParameterEntity.FieldCreateTime + " >= '" + startTime + "'");
            }
            if (ValidateUtil.IsDateTime(endTime))
            {
                sb.Append(" AND " + BaseParameterEntity.FieldCreateTime + " <= DATEADD(s,-1,DATEADD(d,1,'" + endTime + "'))");
            }
        ...
        public DataTable GetDataTable(bool myCompanyOnly = true)
        {
            ...
            if (myCompanyOnly)
            {
                //sb.Append("(" + BaseParameterEntity.FieldUserCompanyId + " = 0 OR " + BaseParameterEntity.FieldUserCompanyId + " = " + UserInfo.CompanyId + ")");
            }
            //return GetDataTable(sb.Put(), null, new KeyValuePair<string, object>(BaseParameterEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseParameterEntity.FieldDeleted, 0));
            ...
            return CacheUtil.Cache<DataTable>(cacheKey, () => GetDataTable(sb.Put(), null, new KeyValuePair<string, object>(BaseParameterEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseParameterEntity.FieldDeleted, 0)), true, false, cacheTime);
        }
