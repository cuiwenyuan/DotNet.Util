...
namespace DotNet.Business
{
    ...
    public partial class BaseDictionaryManager : BaseManager, IBaseManager
    {
        ...
        public DataTable GetDataTableByPage(string companyId, string departmentId, string userId, string startTime, string endTime, string searchKey, out int recordCount, int pageNo = 1, int pageSize = 20, string sortExpression = BaseDictionaryEntity.FieldCreateTime, string sortDirection = "DESC", bool showDisabled = true, bool showDeleted = true)
        {
            ...
            {
                sb.Append(" AND " + BaseDictionaryEntity.FieldCreateTime + " >= '" + startTime + "'");
            }
            if (ValidateUtil.IsDateTime(endTime))
            {
                sb.Append(" AND " + BaseDictionaryEntity.FieldCreateTime + " <= DATEADD(s,-1,DATEADD(d,1,'" + endTime + "'))");
            }
        ...
        public DataTable GetDataTable(bool myCompanyOnly = true)
        {
            ...
            //return GetDataTable(sb.Put(), null, new KeyValuePair<string, object>(BaseDictionaryEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseDictionaryEntity.FieldDeleted, 0));
            ...
            return CacheUtil.Cache<DataTable>(cacheKey, () => GetDataTable(sb.Put(), null, new KeyValuePair<string, object>(BaseDictionaryEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseDictionaryEntity.FieldDeleted, 0)), true, false, cacheTime);
        }
