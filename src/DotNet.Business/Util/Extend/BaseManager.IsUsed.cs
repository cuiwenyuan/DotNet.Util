...
namespace DotNet.Business
{
    ...
    public partial class BaseManager : IBaseManager
    {
        ...
        /// <returns>是否</returns>
        public virtual bool IsUsed(string id, string tableName, string tableFieldName, bool checkUserCompany = false, int tableVersion = 4)
        {
            var result = false;
            if (DbUtil.Exists(dbHelper, tableName) && ValidateUtil.IsInt(id))
            {
                ...
                //未删除
                sb.Append(" AND A." + (tableVersion == 4 ? BaseUtil.FieldDeletionStateCode : BaseUtil.FieldDeleted) + " = 0 AND A.Enabled = 1 ");
                sb.Append(" AND B." + (tableVersion == 4 ? BaseUtil.FieldDeletionStateCode : BaseUtil.FieldDeleted) + " = 0 AND B.Enabled = 1 ");
                //当前用户所在公司或者系统公用数据
                if (checkUserCompany)
                {
        ...
        /// <param name="tableVersion">版本默认4为老版本</param>
        /// <returns>是否</returns>
        public virtual bool IsUsed(string[] ids, string tableName, string tableFieldName, bool checkUserCompany = false, int tableVersion = 4)
        {
            var result = false;
