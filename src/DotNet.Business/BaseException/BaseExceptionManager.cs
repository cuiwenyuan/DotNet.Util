...
namespace DotNet.Business
{
    ...
    public partial class BaseExceptionManager : BaseManager
    {
        ...
        public override DataTable GetDataTableByPage(string companyId, string departmentId, string userId, string startTime, string endTime, string searchKey, out int recordCount, int pageNo = 1, int pageSize = 20, string sortExpression = BaseExceptionEntity.FieldCreateTime, string sortDirection = "DESC", bool showDisabled = true, bool showDeleted = true)
        {
            ...
            {
                sb.Append(" AND " + BaseExceptionEntity.FieldCreateTime + " >= " + dbHelper.ToDbTime(startTime));
            }
            if (ValidateUtil.IsDateTime(endTime))
            {
                sb.Append(" AND " + BaseExceptionEntity.FieldCreateTime + " <= " + dbHelper.ToDbTime(endTime.ToDateTime().Date.AddDays(1).AddMilliseconds(-1)));
            }
        ...
        public DataTable GetDataTable(bool myCompanyOnly = true)
        {
            ...
            //return GetDataTable(sb.Return(), null, new KeyValuePair<string, object>(BaseExceptionEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseExceptionEntity.FieldDeleted, 0));
            ...
            return CacheUtil.Cache<DataTable>(cacheKey, () => GetDataTable(sb.Return(), null, new KeyValuePair<string, object>(BaseExceptionEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseExceptionEntity.FieldDeleted, 0)), true, false, cacheTime);
        }
