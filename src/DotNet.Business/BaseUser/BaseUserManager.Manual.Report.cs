...
namespace DotNet.Business
{
    ...
    public partial class BaseUserManager : BaseManager
    {
        ...
        public DataTable DailyUserReport(int days, string startDate, string endDate)
        {
            ...
            sb.Append(" ,(SELECT COUNT(*) FROM " + CurrentTableName + " A WHERE DATEDIFF(d,A.CreateTime,B.TransactionDate) = 0 AND A." + BaseUtil.FieldEnabled + " = 1 AND A." + BaseUserEntity.FieldDeleted + " = 0) AS TotalNewUserCount");
            ...
            sb.Append(" ,(SELECT COUNT(*) FROM " + BaseOrganizationEntity.CurrentTableName + " A WHERE DATEDIFF(d,A.CreateTime,B.TransactionDate) = 0 AND A." + BaseUtil.FieldEnabled + " = 1 AND A." + BaseOrganizationEntity.FieldDeleted + " = 0 AND ParentId = 0) AS TotalCompanyCount");
