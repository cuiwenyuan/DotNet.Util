...
namespace DotNet.Business
{
    ...
    public partial class BaseOrganizationService : IBaseOrganizationService
    {
        ...
        public bool Exists(BaseUserInfo userInfo, List<KeyValuePair<string, object>> parameters, string id)
        {
            ...
            {
                var manager = new BaseManager(dbHelper, userInfo, BaseOrganizationEntity.CurrentTableName);
            ...
            ServiceUtil.ProcessUserCenterReadDb(userInfo, parameter, (dbHelper) =>
            {
                ...
                if (BaseSystemInfo.OrganizationDynamicLoading)
                {
                    commandText = "    SELECT * "
                                ...
                                + "                OR " + BaseOrganizationEntity.FieldParentId + " IN (SELECT " + BaseOrganizationEntity.FieldId + " FROM " + BaseOrganizationEntity.CurrentTableName + " WHERE " + BaseOrganizationEntity.FieldDeleted + " = 0 AND " + BaseOrganizationEntity.FieldIsInnerOrganization + " = 1 AND " + BaseOrganizationEntity.FieldEnabled + " = 1 AND " + BaseOrganizationEntity.FieldParentId + " IS NULL)) "
