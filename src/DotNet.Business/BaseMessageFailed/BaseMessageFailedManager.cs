...
namespace DotNet.Business
{
    ...
    public partial class BaseMessageFailedManager : BaseManager
    {
        ...
        /// <returns>数据表</returns>
        public override DataTable GetDataTableByPage(string companyId, string departmentId, string userId, string startTime, string endTime, string searchKey, out int recordCount, int pageNo = 1, int pageSize = 20, string sortExpression = BaseMessageFailedEntity.FieldCreateTime, string sortDirection = "DESC", bool showDisabled = true, bool showDeleted = true)
        {
            var sb = PoolUtil.StringBuilder.Get().Append(" 1 = 1");
            //是否显示无效记录
            if (!showDisabled)
            {
                sb.Append(" AND " + BaseMessageFailedEntity.FieldEnabled + " = 1");
            }
            //是否显示已删除记录
            if (!showDeleted)
            {
                sb.Append(" AND " + BaseMessageFailedEntity.FieldDeleted + " = 0");
            }

            if (ValidateUtil.IsInt(companyId))
            {
                //sb.Append(" AND " + BaseMessageFailedEntity.FieldCompanyId + " = " + companyId);
            }
            // 只有管理员才能看到所有的
            if (!(UserInfo.IsAdministrator && BaseSystemInfo.AdministratorEnabled))
            {
            sb.Append(" AND (" + BaseMessageFailedEntity.FieldUserCompanyId + " = 0 OR " + BaseMessageFailedEntity.FieldUserCompanyId + " = " + UserInfo.CompanyId + ")");
            }
            if (ValidateUtil.IsInt(departmentId))
            {
                //sb.Append(" AND " + BaseMessageFailedEntity.FieldDepartmentId + " = " + departmentId);
            }
            if (ValidateUtil.IsInt(userId))
            {
                //sb.Append(" AND " + BaseMessageFailedEntity.FieldUserId + " = " + userId);
            }
            //创建时间
            if (ValidateUtil.IsDateTime(startTime))
            {
                sb.Append(" AND " + BaseMessageFailedEntity.FieldCreateTime + " >= " + dbHelper.ToDbTime(startTime));
            }
            if (ValidateUtil.IsDateTime(endTime))
            {
                sb.Append(" AND " + BaseMessageFailedEntity.FieldCreateTime + " <= " + dbHelper.ToDbTime(endTime.ToDateTime().Date.AddDays(1).AddMilliseconds(-1)));
            }
        ...
        public DataTable GetDataTable(bool myCompanyOnly = true)
        {
            ...
            //return GetDataTable(sb.Return(), null, new KeyValuePair<string, object>(BaseMessageFailedEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseMessageFailedEntity.FieldDeleted, 0));
            ...
            return CacheUtil.Cache<DataTable>(cacheKey, () => GetDataTable(sb.Return(), null, new KeyValuePair<string, object>(BaseMessageFailedEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseMessageFailedEntity.FieldDeleted, 0)), true, false, cacheTime);
        }
