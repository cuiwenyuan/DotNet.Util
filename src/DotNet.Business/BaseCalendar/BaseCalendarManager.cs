//-----------------------------------------------------------------------
// <copyright file="BaseCalendarManager.cs" company="DotNet">
...
namespace DotNet.Business
{
    ...
    public partial class BaseCalendarManager : BaseManager
    {
        ...
        /// <returns>数据表</returns>
        public override DataTable GetDataTableByPage(string companyId, string departmentId, string userId, string startTime, string endTime, string searchKey, out int recordCount, int pageNo = 1, int pageSize = 20, string sortExpression = BaseCalendarEntity.FieldCreateTime, string sortDirection = "DESC", bool showDisabled = true, bool showDeleted = true)
        {
            var sb = PoolUtil.StringBuilder.Get().Append(" 1 = 1");
            //是否显示无效记录
            if (!showDisabled)
            {
                sb.Append(" AND " + BaseCalendarEntity.FieldEnabled + " = 1");
            }
            //是否显示已删除记录
            if (!showDeleted)
            {
                sb.Append(" AND " + BaseCalendarEntity.FieldDeleted + " = 0");
            }
            ...
            {
                //sb.Append(" AND " + BaseCalendarEntity.FieldDepartmentId + " = " + departmentId);
            }
            if (ValidateUtil.IsInt(userId))
            {
                //sb.Append(" AND " + BaseCalendarEntity.FieldUserId + " = " + userId);
            }
            //创建时间
            if (ValidateUtil.IsDateTime(startTime))
            {
                sb.Append(" AND " + BaseCalendarEntity.FieldCreateTime + " >= " + dbHelper.ToDbTime(startTime));
            }
            if (ValidateUtil.IsDateTime(endTime))
            {
                sb.Append(" AND " + BaseCalendarEntity.FieldCreateTime + " <= " + dbHelper.ToDbTime(endTime.ToDateTime().Date.AddDays(1).AddMilliseconds(-1)));
            }
            if (!string.IsNullOrEmpty(searchKey))
            {
                searchKey = StringUtil.GetLikeSearchKey(dbHelper.SqlSafe(searchKey));
                sb.Append(" AND (" + BaseCalendarEntity.FieldFiscalYear + " LIKE N'%" + searchKey + "%' OR " + BaseCalendarEntity.FieldFiscalMonth + " LIKE N'%" + searchKey + "%')");
            }
            sb.Replace(" 1 = 1 AND ", " ");
        ...
        public DataTable GetDataTable(bool myCompanyOnly = true)
        {
            ...
            if (myCompanyOnly)
            {
                sb.Append("(" + BaseCalendarEntity.FieldUserCompanyId + " = 0 OR " + BaseCalendarEntity.FieldUserCompanyId + " = " + UserInfo.CompanyId + ")");
            }
            //return GetDataTable(sb.Return(), null, new KeyValuePair<string, object>(BaseCalendarEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseCalendarEntity.FieldDeleted, 0));
            ...
            return CacheUtil.Cache<DataTable>(cacheKey, () => GetDataTable(sb.Return(), null, new KeyValuePair<string, object>(BaseCalendarEntity.FieldEnabled, 1), new KeyValuePair<string, object>(BaseCalendarEntity.FieldDeleted, 0)), true, false, cacheTime);
        }
