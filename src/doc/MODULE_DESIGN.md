...
- 重要类/接口：
  ...
  - `Msg`（2026-09 新增）：多语言输出消息层，配合 `Msg.Typed.cs` 强类型入口与 `MsgPack.zh-CN/en` 内嵌语言包，默认 zh-CN、内置 en，共 400 条；取值绝不抛异常。
...
- 消息文本（2026-09 新增约定）：面向用户的异常/状态/日志/控制台文本一律走 `Msg` 强类型成员（如 `Msg.Common.UnknownError`、`Msg.Common.ParameterRequired(x)`），禁止硬编码中文；需指定语言时用 `Msg.Get(key, culture)`。
