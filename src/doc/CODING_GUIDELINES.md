...
18. 输出消息与多语言（2026-09-18 新增）
- **面向用户的输出消息（异常文本、业务状态、日志、控制台输出）一律走 `Msg` 层**，禁止在代码中硬编码中文字符串。
- **优先使用强类型成员**（`src/DotNet.Util/Message/Msg.Typed.cs`，400 条全覆盖）：
  - 无 `{n}` 占位符 → 属性：`Msg.Common.UnknownError`
  - 含 `{n}` 占位符 → 方法：`Msg.Common.ParameterRequired(fieldName)`
  - 枚举描述：`Msg.Enum.Status.Ok`
- 仅在**键在编译期未知**（配置驱动、插件、按类型名拼键）或**需显式指定语言**时使用字符串键：
  `Msg.Get(key)` / `Msg.Get(key, culture)` / `Msg.Format(key, args)` / `Msg.GetOrDefault(key, fallback)`。
- **禁止再使用 `Msg####` 编号键**：2026-09-16 起已从语言包移除，写错不报错、只静默返回键名本身。
  旧编号键的语义对应关系查 `Msg-Key-Rename-Map.md`。
- **禁止用 `AppMessage.Msg####` 字段值做多语言输出**：这些字段恒为中文、保留只为二进制兼容；
  字段值仍可作为「默认值/回退值」使用。
- 新增词条时须**中英语言包同时登记**（`MsgPack.zh-CN.cs` 与 `MsgPack.en.cs` 键集合须一致，单测会断言），
  并同步在 `Msg.Typed.cs` 增加强类型成员。
- 消息键**区分大小写**（Ordinal），语言名不区分（culture 用 OrdinalIgnoreCase）。
- ?? `DotNet.Web.UI.BasePage` 包内另有同名类型 `DotNet.Util.Msg`（WebForm 弹窗辅助类）。
  两个包同时引用时裸写 `Msg` 会二义，请用命名空间别名区分，勿在 `DotNet.Util` 内再建同名类型。
- 语言切换是**全局静态状态**：任何断言默认中文的测试类必须标注
  `[Collection(MsgTestCollection.Name)]`（`DisableParallelization = true`），否则并行竞争会产生偶发失败。

19. 文件编码与换行
- **所有文本文件（.md / .cs / .csproj / .json / .yml 等）一律 UTF-8 无 BOM**。
  历史遗留的 `src/doc/*.md` 曾为 GBK，已于 2026-09-18 统一转 UTF-8（换行保持原样，未做 CRLF/LF 归一）。
- 不要手动改回 GBK 或用「另存为 ANSI」保存，否则 GitHub 与 Linux 侧会乱码。
- 换行符不做强制统一（仓库内 CRLF 与 LF 并存）；**不要新增 `.gitattributes` 的 `eol=crlf` 规则**——
  会把现存 LF 文件在 checkout 时整体改写，产生大量无意义 diff。
